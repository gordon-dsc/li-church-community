...
```{r}
#wrangle household data
li_household_data <- tibble(li_household_data)
li_household_data |>
  unnest_longer(col = li_household_data)|>
  unnest_wider(col = li_household_data, names_sep = "_")|>
  distinct()|>
  row_to_names(1)|>
  select(-ends_with("A")) -> household_testing
```
