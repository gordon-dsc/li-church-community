...
closest_bg <- function(lat1, lon1, lat2, lon2){
  df <- tibble(lat1, lon1, lat2, lon2)
  
  df|>
    group_by(lat1, lon1)|>
    mutate(distance = euclidian_distance(lat1, lon1, lat2, lon2))|>
    arrange(distance)|>
    slice_max(n = 1)|>
    ungroup()
}
...
```{r}
testing <- closest_bg(long_island_bg$intptlat, long_island_bg$intptlon, map_data$lat, map_data$lon)
```
