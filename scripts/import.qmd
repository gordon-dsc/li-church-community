...
census_request <- tibble(geoid = geoids) |>
  tidyr::crossing(group = dhc_groups)
...
census_dhc_data <- pmap_dfr(
  list(census_request$geoid, census_request$group),
  ~ get_dhc_by_geoid(..1, ..2, key = "YOUR_KEY"))
```
